...
from .errors import MulticraftException
from .model import (
    Host,
    User,
    Role,
...
class MulticraftAPI:
    def __init__(
        self, host: Host, user: Optional[str] = None, key: Optional[str] = None, logger: bool = False
    ):
        """
        Create a new instance of Multicraft API

        :param host: The multicraft host
        :type host: Host
        :param user: The username of the account to use
        :type user: str
        :param key: The API key to login with, defaults to MULTICRAFT_KEY
        :type key: str, optional
        """
        self.host = host
        self._user = str(user) if user else os.getenv("MULTICRAFT_USER")
        self._key = str(key) if key else os.getenv("MULTICRAFT_KEY")
        self._session = requests.Session()
        self._session.headers = {"user-agent": "multicraft.py"}
        self.logger = logging.getLogger(self.__class__.__name__)
        if not logger:
            self.logger.disabled = True
        self.connect()

    def __enter__(self) -> 'MulticraftAPI':
        return self
    ...
    # username
    # password
    def get_file_manager(self, username: str, password: str = "", **kw):
        """
        Get the sftp connection to the server. Requires 'pysftp' module

        :param username: The sftp username
        :type username: str, optional
        :param password: The sftp password, defaults to SFTP_PASSWORD
        :type password: str, optional
        :rtype: FileManager
        """
        from .file_manager import FileManager

        return FileManager(self.host, username, password, **kw)

    # Internal

    def _generateSignature(self, params: dict[str, Any]) -> str:
        signature = ""
        for param in self._reduce(params):
            signature += param + str(params[param])

        return hmac.new(
            str(self._key).encode("UTF-8"), str(signature).encode(), hashlib.sha256
        ).hexdigest()

    def _reduce(self, obj: dict[str, Any]) -> Dict[str, Any]:
        res = {}
    ...
    def _call(self, method: str, params: dict[str, Any] = {}) -> Dict[str, Any]:
        ...
        """
        if self.logger:
            self.logger.info("Triggered event " + method)
        params = self._reduce(params)
        params["_MulticraftAPIMethod"] = str(method)
        params["_MulticraftAPIUser"] = self._user
        params["_MulticraftAPIKey"] = self._generateSignature(params)
        res = self._session.post(self.host.api_url, params)
    ...
    def connect(self) -> None:
        ...
        # owner = self.get_user_id(self._user)
        # print(owner)
    ...
    def set_user_agent(self, user_agent_string: str) -> None:
        ...
        """
        self._session.headers["user-agent"] = str(user_agent_string)
        return None
    ...
    def get_current_user(self) -> User:
        ...
        user = User.model_validate(self, res["User"])
        setattr(user, "_api", self)
        return user

    def get_user_role(self, user_id: int, server_id: int) -> Role:
        """
        Get the users role
        ...
        :rtype: Role|None
        """
        data = {"user_id": int(user_id), "server_id": int(server_id)}
    ...
    def set_user_role(self, user_id: int, server_id: int, role: Role) -> None:
        ...
        if not isinstance(role, Role) and role is not None:
            raise TypeError(
                f"Expected Role but got '{role.__class__.__name__}' instead"
            )
        data = {
            "user_id": int(user_id),
    ...
    def get_user_ftp_access(self, user_id: int, server_id: int) -> Mode:
        """
        ...
        """
        res = self._call(
            "getUserFtpAccess", {"user_id": int(user_id), "server_id": int(server_id)}
        )["mode"]
        if res == "":
            return Mode.ro
        return Mode[str(res)]
    ...
    def set_user_ftp_access(self, user_id: int, server_id: int, mode: Mode) -> None:
        ...
        if not isinstance(mode, Mode) and mode is not None:
            raise TypeError(
                f"Expected Mode but got '{mode.__class__.__name__}' instead"
            )
        self._call(
            "setUserFtpAccess",
            {
    ...
    def get_user_id(self, name: str) -> int:
        ...
        :rtype: int
        """
        return self._call("getUserId", {"name": str(name)})["id"]

    # TODO - Incorrect API key from user: USERNAME
    def get_own_api_key(
        self, password: str, generate=0, gauth_code: str = ""
    ) -> Dict[str, Any]:
        """
        _summary_
    ...
    def list_players(self, server_id: int) -> Dict[str, str]:
        ...
        :rtype: dict
        """
        res = self._call("listPlayers", {"server_id": int(server_id)})["Players"]
    ...
    def get_player(self, player_id: int) -> Player:
        ...
        :rtype: Player
        """
        res = self._call("getPlayer", {"id": int(player_id)})["Player"]
        player = Player.model_validate(self, res)
        setattr(player, "_api", self)
        return player

    def find_players(
        self, server_id: int, field: list[str], value: list[str]
    ) -> Dict[str, str]:
        """
        ...
        """
        res = self._call(
            "findPlayers", {"server_id": int(server_id), "field": field, "value": value}
        )["Players"]
        return {} if isinstance(res, list) else res
    ...
    def update_player(self, player_id: int, field: list[str], value: list[str]) -> None:
        ...
        """
        self._call(
            "updatePlayer", {"id": int(player_id), "field": field, "value": value}
        )
        return None
    ...
    def delete_player(self, player_id: int) -> Dict[str, Any]:
        ...
        :rtype: _type_
        """
        return self._call("deletePlayer", {"id": int(player_id)})
    ...
    def assign_player_to_user(self, player_id: int, user_id: int) -> None:
        ...
        """
        self._call(
            "assignPlayerToUser", {"player_id": int(player_id), "user_id": int(user_id)}
        )
        return None
    ...
    def list_commands(self, server_id: int) -> Dict[str, str]:
        ...
        :rtype: dict
        """
        res = self._call("listCommands", {"server_id": int(server_id)})["Commands"]
        return {} if isinstance(res, list) else res
    ...
    def find_commands(
        ...
        res = self._call(
            "findCommands",
            {"server_id": int(server_id), "field": field, "value": value},
        )["Commands"]
        return {} if isinstance(res, list) else res
    ...
    def get_command(self, command_id: int) -> Command:
        ...
        :rtype: Command
        """
        res = self._call("getCommand", {"id": int(command_id)})["Command"]
        command = Command.model_validate(self, res)
        setattr(command, "_api", self)
        return command

    def update_command(
        self, command_id: int, field: list[str], value: list[str]
    ) -> None:
        """
        Update this command
        ...
        """
        self._call(
            "updateCommand", {"id": int(command_id), "field": field, "value": value}
        )
        return None

    def create_command(
        self, server_id: int, name: str, role: Role, chat: str, response: str, run: str
    ) -> Command:
        """
    ...
    def delete_command(self, command_id: int) -> None:
        ...
        :type command_id: int
        """
        self._call("deleteCommand", {"id": int(command_id)})
        return None
    ...
    def list_servers_by_owner(self, user_id: int) -> Dict[str, str]:
        ...
        :rtype: dict
        """
        res = self._call("listServersByOwner", {"user_id": int(user_id)})["Servers"]
        return {} if isinstance(res, list) else res

    # TODO - Access denied
    def list_servers_by_connection(self, connection_id: int) -> Dict[str, Any]:
        res = self._call(
            "listServersByConnection", {"connection_id": int(connection_id)}
        )
        return res
    ...
    def update_server(
        ...
        """
        return self._call(
            "updateServer", {"id": int(server_id), "field": field, "value": value}
        )
    ...
    def start_server(self, server_id: int) -> None:
        ...
        :type server_id: int
        """
        self._call("startServer", {"id": int(server_id)})
        return None
    ...
    def stop_server(self, server_id: int) -> None:
        ...
        :type server_id: int
        """
        self._call("stopServer", {"id": int(server_id)})
        return None
    ...
    def restart_server(self, server_id: int) -> None:
        ...
        :type server_id: int
        """
        self._call("restartServer", {"id": int(server_id)})
        return None
    ...
    def kill_server(self, server_id: int) -> None:
        ...
        :type server_id: int
        """
        self._call("killServer", {"id": int(server_id)})
        return None
    ...
    def send_console_command(self, server_id: int, command: str) -> None:
        ...
        """
        self._call(
            "sendConsoleCommand", {"server_id": int(server_id), "command": str(command)}
        )
        return None
    ...
    def clear_server_log(self, server_id: int) -> None:
        ...
        :type server_id: int
        """
        self._call("clearServerLog", {"id": int(server_id)})
        return None
    ...
    def clear_server_chat(self, server_id: int) -> None:
        ...
        :type server_id: int
        """
        self._call("clearServerChat", {"id": int(server_id)})
        return None
    ...
    def get_server(self, server_id: int) -> Server:
        ...
        :rtype: Server
        """
        res = self._call("getServer", {"id": server_id})
        res["Server"]["id"] = server_id
        server = Server.model_validate(self, res["Server"])
        setattr(server, "_api", self)
        return server

    def get_server_status(
        self, server_id: int, player_list: bool = False
    ) -> ServerStatus:
        """
        ...
        :rtype: ServerStatus
        """
        if not isinstance(player_list, bool):
            raise TypeError(
                f"Expected bool but got '{player_list.__class__.__name__}' instead"
            )
        res = self._call(
            "getServerStatus",
            {"id": int(server_id), "player_list": 1 if player_list else 0},
        )
        status = ServerStatus.model_validate(self, res)
        setattr(status, "_api", self)
        return status
    ...
    def get_server_log(self, server_id: int) -> List[str]:
        ...
        :rtype: list[str]
        """
        res = self._call("getServerLog", {"id": int(server_id)})
    ...
    def get_server_chat(self, server_id: int) -> List[ChatMessage]:
        ...
        """
        res = self._call("getServerChat", {"id": int(server_id)})
        msgs = [ChatMessage.model_validate(self, x) for x in res]
        for msg in msgs:
            setattr(msg, "_api", self)
        return msgs
    ...
    def get_server_resources(self, server_id: int) -> ServerResources:
        ...
        """
        res = self._call("getServerResources", {"id": int(server_id)})
        resources = ServerResources.model_validate(self, res)
        setattr(resources, "_api", self)
        return resources
    ...
    def list_schedules(self, server_id: int) -> Dict[str, str]:
        ...
        :rtype: dict
        """
        res = self._call("listSchedules", {"server_id": int(server_id)})["Schedules"]
        return {} if isinstance(res, list) else res
    ...
    def update_schedule(
        self, schedule_id: int, field: list[str], value: list[str]
    ) -> None:
        """
        ...
        """
        self._call(
            "updateSchedule", {"id": int(schedule_id), "field": field, "value": value}
        )
        return None
    ...
    def get_schedule(self, schedule_id: int) -> Schedule:
        ...
        :rtype: Schedule
        """
        res = self._call("getSchedule", {"id": int(schedule_id)})["Schedule"]
        schedule = Schedule.model_validate(self, res)
        setattr(schedule, "_api", self)
        return schedule
    ...
    def create_schedule(
        ...
        if not isinstance(ts, datetime.datetime):
            raise TypeError(
                f"Expected datetime.datetime but got '{ts.__class__.__name__}' instead"
            )
        if not isinstance(status, ScheduleStatus):
            raise TypeError(
                f"Expected ScheduleStatus but got '{ts.__class__.__name__}' instead"
            )
        data = {
            "server_id": int(server_id),
            "name": str(name),
    ...
    def delete_schedule(self, schedule_id: int) -> None:
        ...
        :type schedule_id: int
        """
        self._call("deleteSchedule", {"id": int(schedule_id)})
        return None
    ...
    def get_database_info(self, server_id: int) -> Database:
        ...
        :rtype: Database
        """
        res = self._call("getDatabaseInfo", {"server_id": int(server_id)})
        database = Database.model_validate(self, res)
        setattr(database, "_api", self)
        return database

    def create_database(
        self, server_id: int, name: Optional[str] = None, password: Optional[str] = None
    ) -> Database:
        """
    ...
    def change_database_password(
        self, server_id: int, database_id: Optional[int] = None, password: Optional[str] = None
    ) -> Database:
        """
        ...
        res = self._call(
            "changeDatabasePassword",
            {
                "server_id": int(server_id),
                "database_id": (
                    int(server_id) if database_id is None else int(database_id)
                ),
                "password": str(password),
            },
        )
    ...
    def delete_database(self, server_id: int, database_id: Optional[int] = None) -> 'MulticraftAPI':
        """
        Remove a database
    ...
    def start_server_backup(self, server_id: int) -> 'MulticraftAPI':
        """
        ...
        :type server_id: int
        """
        self._call("startServerBackup", {"id": int(server_id)})
        return self
    ...
    def get_server_backup_status(self, server_id: int) -> Backup:
        ...
        :rtype: Backup
        """
        res = self._call("getServerBackupStatus", {"id": int(server_id)})
        backup = Backup.model_validate(self, res)
        setattr(backup, "_api", self)
        return backup
